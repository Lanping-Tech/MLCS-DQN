...
env = Environment('data.txt')
N_ACTIONS = len(env.mlcs.charset)  # 4 actions
N_STATES = len(env.mlcs.seqs) * len(env.mlcs.charset) # 4 states
...
class DQN(object):
    ...
    def  choose_action(self, x):
        ...
        if np.random.uniform() < EPSILON:   # greedy
            ...
            action = int(torch.argmax(actions_value).data.numpy())
        else:   # random
    ...
    def store_transition(self, s, a, r, s_):
        ...
        a_r = np.array([a, r]).reshape(1, 2)    
        transition = np.hstack((s, a_r, s_)) # horizontally stack these vectors
...
plt.plot(np.arange(len(env.levels)), env.levels)
plt.xlabel('episode')
plt.ylabel('level')
plt.show()